...
public class MuteCommand extends Command implements Listener {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if(BungeeManagement.getAdminController().getDatabase().getMuteReason(args[0]) != null){
            player.sendMessage(new TextComponent("NMSL"));
            return;
        }

        if(args.length == 3){
            if(args[1].toLowerCase().matches("([smhdwy]|mon|second|minute|hour|day|week|month|year|seconds|minutes|hours|days|weeks|months|years)")){
                player.sendMessage(new TextComponent("§c格式错误!"));
                return;
            }
            ...
            player.sendMessage(new TextComponent("§a禁言成功!"));
            return;
        }
    ...
    private void sendCommand(String admin, String name, String date, String reason) {
        //操作者，禁言人，时间，解释
        BungeeManagement.getInstance().getProxy().getScheduler().runAsync(BungeeManagement.getInstance(), () -> BungeeManagement.getInstance().getRedisBungeeAPI().sendChannelMessage("Management", "Mute|" + admin + "|" + name + "|" + date + "|" + reason));
    }
        ...
        BungeeManagement.getInstance().getProxy().getScheduler().runAsync(BungeeManagement.getInstance(), () -> {
            ...
            if (action.equals("Mute")) {
                ...
                if(BungeeManagement.getInstance().getProxy().getPlayer(name) != null){
                    BungeeManagement.getInstance().getProxy().getPluginManager().dispatchCommand(BungeeManagement.getInstance().getProxy().getConsole(), "tempban " + name + " " + date + " §b" + BungeeManagement.getAdminController().getDatabase().getAdmin(admin).getDisplayName() + "§f|" + reason);
                }

                BungeeManagement.getInstance().getProxy().broadcast(new TextComponent("§c全服禁言:§e" + name + "§c,操作者:§e" + BungeeManagement.getAdminController().getDatabase().getAdmin(admin).getDisplayName() + ",§c原因:§e" + reason));
            }
        });
